...
La implementación naive cae en una trampa de concurrencia. La forma correcta invierte el orden. Compara las dos:

<Tabs defaultValue="roto">
  <TabsList>
    <TabsTrigger value="roto">SELECT-then-INSERT (roto)</TabsTrigger>
    <TabsTrigger value="correcto">INSERT-then-catch (correcto)</TabsTrigger>
  </TabsList>
...
El de la izquierda **no funciona** bajo concurrencia. Dos requests llegando al mismo tiempo pueden ambos pasar el `SELECT` (porque ninguno ha insertado todavía), ambos ejecutar la transacción, y ambos intentar insertar al final. Resultado: dos transacciones reales, no una.

El de la derecha **intenta el INSERT atómico primero, y atrapa la violación de UNIQUE constraint**. PostgreSQL garantiza que solo un request gana el INSERT. Los demás reciben el error 23505 y entran al flujo de manejo de duplicados.
...
**El lock con timeout es crítico.** Si un request adquiere el lock (`status=PROCESSING`) pero nunca completa —el proceso muere por OOM, el container se reinicia, lo que sea— el `locked_at` permite que requests subsecuentes re-adquieran el lock después de un timeout (un buen default es 30 segundos). Sin esto, el sistema entra en deadlock permanente: la key existe en `PROCESSING` para siempre, y todos los retries del cliente reciben 409 Conflict eternamente.
</Callout>
